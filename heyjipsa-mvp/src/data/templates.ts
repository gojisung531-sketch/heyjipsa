// 가구 유형별 집안일 템플릿
// → household-checklist/scripts/templates.py 에서 그대로 포팅 (한국어 데이터 보존)
//
// 각 항목 구조: { id, name, category }
// 주기 키: daily / weekly / monthly / seasonal
...
export type PeriodMap = Record<Period, TemplateItem[]>;

// ─────────────────────────────────────────────
// 공통 (모든 가구)
// ─────────────────────────────────────────────
const COMMON: PeriodMap = {
  daily: [
...
const SINGLE: PeriodMap = {
  ...
  monthly: [{ id: 'single_bills', name: '공과금·관리비 확인', category: '1인' }],
  seasonal: [],
};
...
// 2인 가구 (동거·부부, 아이 없음)
// ─────────────────────────────────────────────
const COUPLE: PeriodMap = {
  daily: [{ id: 'couple_share', name: '오늘 집안일 분담 확인', category: '2인' }],
  weekly: [
...
// 식물
// ─────────────────────────────────────────────
const PLANT: PeriodMap = {
  daily: [{ id: 'plant_check', name: '잎 상태 확인 (시들음·벌레)', category: '식물' }],
  weekly: [
...
// 카테고리 매핑
export const CATEGORY_TEMPLATES: Record<string, PeriodMap> = {
  common: COMMON,
  single: SINGLE,
  couple: COUPLE,
...
export const PERIODS: Period[] = ['daily', 'weekly', 'monthly', 'seasonal'];
...
// 체크리스트 탭 라벨 (UI 표기: 오늘 / 이번 주 / 이번 달 / 계절)
export const PERIOD_TAB_LABELS: Record<Period, string> = {
  daily: '오늘',
  weekly: '이번 주',
  monthly: '이번 달',
  seasonal: '계절',
