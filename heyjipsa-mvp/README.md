...
- 음성 입력(🎤): 웹은 브라우저 SpeechRecognition, 네이티브는 @capacitor-community/speech-recognition (ko-KR)
...
5. **집안일 체크리스트** `/checklist` — 오늘/이번 주/이번 달/계절 탭 + 진행률 + **내 할일**(🎤음성·자연어 캡처)

> 🎤 **음성 입력(시리처럼)**: 모든 앱 화면 우하단 마이크 버튼 → 말하면 자연어를 분석해 할 일로 자동 정리·추가. 내 할일·가사분담 기록 입력에도 음성 버튼 제공.
